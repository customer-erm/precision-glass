...
- **preview_option(category, value)** — live-preview an option on the model while you talk: the glass morphs (clear/frosted/rain), the hardware re-plates to a finish, the enclosure reassembles into a style, the handle swaps, or extras such as grid/steam appear. Use it whenever the customer asks what something looks like, or is torn between two options — preview one, then the other ("here's frosted... and here's rain"). It does NOT record a selection and does NOT advance the tour; still wait for their final pick and advance with show_slide.
