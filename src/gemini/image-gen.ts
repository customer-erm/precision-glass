...
export async function generateShowerImage(
  ...
  if (userBathroom) {
    ...
    promptParts.push({
      text: `${prompt}\n\nIMPORTANT: The reference image of the customer\u2019s ACTUAL BATHROOM is attached. Render the new frameless shower INTO that exact bathroom — preserve their tile, their fixtures, their window, their vanity, their lighting.${bathroomAnalysis ? `\n\nINSTALLER PHOTO READ: ${bathroomAnalysis} Use this as practical layout guidance, but let the actual pixels win if there is a conflict.` : ''}\n\nSCOPE OF WORK — STRICT, READ CAREFULLY: This is a drop-in glass enclosure quote, NOT a remodel. The ONLY change permitted is installing frameless glass at the existing wet area. Specifically: (a) place the glass ONLY across the opening of the EXISTING tub or EXISTING shower enclosure — the footprint already framed by the surrounding walls/curb — spanning that opening and nothing more; (b) the glass is a thin enclosure at the threshold, NOT a wall covering — do NOT lay glass flat against or across any wall, do NOT extend glass along a wall beyond the wet-area opening, do NOT turn a tiled wall into a glass wall; (c) do NOT alter the STRUCTURE of the room in ANY way — do NOT move, add, remove, resize, or re-tile any wall, partition, alcove, niche, window, door, ceiling, soffit, vanity, toilet, mirror, lighting, or plumbing fixture; wall positions and room geometry are FIXED; (d) everything outside the existing tub/shower footprint stays 100% unchanged and pixel-identical.\n\nTUB-TO-SHOWER CONVERSION: If the photo shows a BATHTUB in the wet area, REMOVE the tub completely — this is a tub-to-shower conversion. Build the new frameless shower STRICTLY within the tub's existing footprint: a low tiled curb or low-profile base where the tub apron was, wall tile extended down to the new base, and the glass enclosure spanning the former tub opening only. No part of the tub may remain visible, and the new shower must not extend beyond where the tub already was.\n\nCRITICAL PIXEL ALIGNMENT FOR BEFORE/AFTER: The output must match the input photo EXACTLY — identical camera position, lens, framing, crop, aspect ratio, perspective, and lighting. Every element that is not part of the new shower (walls, floor outside the wet area, window, vanity, mirror, door, decor) must stay pixel-identical to the reference so a before/after overlay aligns 100%. Do not zoom, shift, rotate, or re-crop the scene.`,
    });
